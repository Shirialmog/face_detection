...
from face_detection.prepare_data.create_train_df import create_unique_train_set_df
from torch_intermediate_layer_getter import IntermediateLayerGetter as MidGetter
from face_detection.training.dataset import get_datasets
from face_detection.training.dataset import TestDataset
from face_detection.training.model import load_model
...
def train_vgg(config, df_path, weights_dir):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        #writer.add_scaler('lr', optimizer.param_groups[0]['lr'], epoch)
        ...
        epoch_val_mean_cos, epoch_val_thresh_cos = [], []
        for i, data in enumerate(test_dataloader, 0):
            ...
            epoch_val_mean_cos.append(cos_result.detach().numpy())
            epoch_val_thresh_cos.append((pos==label).detach().numpy())

        print(f'[validation epoch: {epoch + 1}/{epochs}] loss: {np.mean(epoch_val_mean_cos):.3f}, acc: {np.mean(epoch_val_thresh_cos)}')

        #writer.add_scalar('Loss/val', np.mean(np.array(epoch_val_loss)), epoch)
       # writer.add_scalar('Accuracy/val', np.mean(np.array(epoch_val_loss)), epoch)
